...
useEffect(() => {
  ...
  if (!storedEmployeeId) {
    console.warn("No employeeId found in localStorage. Redirecting to sign-in.");
    // Redirect to login or show a friendly message
    window.location.href = "/signin"; // or use navigate("/signin") if inside a router
    return;
  }
...
const loadData = async () => {
  ...
  /*const handleSendPaymentRequest = async () => {
    try {
      ...
      const activeTransactions = transactions.filter(t => t.status === "ACTIVE"|| t.status === "GENERATED");
      await Promise.all(
        activeTransactions.map(t => 
          transactionApi.updateStatus(t.id, "PAID")
        )
      );
 ...
 const handleSendPaymentRequest = async () => {
  // Check for any blocking statuses
  if (billPreview.hasModified) {
    setNotification({
      type: "error",
      message: "Cannot pay with modified transactions. Please resolve with admin."
    });
    return;
  }

  if (billPreview.allPaid) {
    setNotification({
      type: "warning",
  ...
  // Only allow payment if ALL transactions are GENERATED
  if (billPreview.activeCount > 0) {
    setNotification({
      type: "error",
      message: "Bill not fully generated by admin yet."
    });
    return;
  }

  // Show success dialog instead of actually processing payment
  setNotification({
    type: "success",
                  ...
                  <tr key={transaction.id} 
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex space-x-2">
                        ...
                        {["ACTIVE", "GENERATED", "INACTIVE", "MODIFIED"].includes(transaction.status) && (
  <button
    onClick={() => {
      setSelectedTransaction(transaction);
      setShowRemarkDialog(true);
...
<td className="px-6 py-4 whitespace-nowrap text-sm">
  {(transaction.remarks || transaction.responses) ? (
    <button
      onClick={() => showConversation(transaction)}
        ...
        <Edit3 className="w-4 h-4" />
              ...
              <div className="flex justify-end space-x-2 pt-4">
               ...
               {/*} <button
                  ...
                  disabled={transactions.filter(t => t.status === "ACTIVE"||t.status === "GENERATED").length === 0}
                  className={`flex items-center px-4 py-2 rounded-md text-white ${
                    transactions.filter(t => t.status === "ACTIVE"||t.status === "GENERATED").length === 0
